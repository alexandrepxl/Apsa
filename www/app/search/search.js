angular.module('app.search', ['ngCordova.plugins.toast'])
    ...
    .controller('SearchResultsCtrl', function($scope, $state, $stateParams, $ionicLoading, $ionicHistory, $ionicModal, FilterService, Location, LocationService, Search, Favorites, Searches) {
        ...
        $scope.populateList = function(my_latitude, my_longitude) {
                        ...
                        for (c = 0; c < responseArr.length; c++) {
                            ...
                            var lat_imovel = responseArr[i].CoordenadasGeograficas[0].replace(',','.');
                            var long_imovel =  responseArr[i].CoordenadasGeograficas[1].replace(',','.');
    ...
    .controller('SearchMapCtrl', function($scope, $state, $ionicLoading, $stateParams, $ionicHistory,ResultsInMap,$ionicModal, Location ,Search, FilterService) {
        ...
        var setMap = function(index, points, code, price) {
                ...
                google.maps.event.addListener(marker, "mousedown", function (e) {
                 ...
                 google.maps.event.addListener(map, "mousedown", function(event) {
                        ...
                        console.log("coordenadas do click " + get_lat + "," + get_lng);
                 }); 
        ...
        location.getPosition().then(function() {
           ...
           if(get_lat == "" && get_lng == ""){
                 ...
                 Search.loadData().then(function(){
                    ...
                    googleMap(get_lat,get_lng); 
              

                     for (i = 0; i < responseArr.length; i++) {

                          latitude = responseArr[i].CoordenadasGeograficas[0].replace(',','.');
                          longitude = responseArr[i].CoordenadasGeograficas[1].replace(',','.');
                          ...
                          // Chamo a função setMap para criar os pins
        ...
        location.getPosition().then(function() {
            ...
            location.getReverse(lat,long).then(function(results) {
                 ...
                 Search.loadData().then(function(){
                     ...
                     googleMap(responseArr[0].CoordenadasGeograficas[0].replace(',','.'),responseArr[0].CoordenadasGeograficas[1].replace(',','.'));
