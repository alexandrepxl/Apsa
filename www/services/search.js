...
.service('FilterService', function($soap, $q) {
  ...
  var loadRealtyTypes = function() {
    ...
    $soap.post(base_url,method).then(function(response) {


      if (response.ArrayOfFiltroTipoImovelAluguel !== undefined) {
          _realtyTypes = response.ArrayOfFiltroTipoImovelAluguel;

      }else if(response.ArrayOfFiltroTipoImovelVenda !== undefined){
          _realtyTypes = response.ArrayOfFiltroTipoImovelVenda;
      }
